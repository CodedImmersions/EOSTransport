...
namespace EpicTransport
{
    public class TransportLogger : MonoBehaviour
    {
        ...
        private void OnLog(ref LogMessage msg)
        {
            ...
            if (msg.Message.ToString().Contains("Error response received from backend."))
            {
#if UNITY_EDITOR
                Debug.LogError($"<color=#00b7ff>[EOSTransport]</color> Level: {msg.Level}, Category: {msg.Category}, Message: {msg.Message}");
#else
                Debug.LogError($"[EOSTransport] Level: {msg.Level}, Category: {msg.Category}, Message: {msg.Message}");
#endif
                return;
            }
            ...
            switch (msg.Level)
            {
                ...
                case LogLevel.Info:
#if UNITY_EDITOR
                    Debug.Log($"<color=#00b7ff>[EOSTransport]</color> Level: {msg.Level}, Category: {msg.Category}, Message: {msg.Message}");
#else
                    Debug.Log($"[EOSTransport] Level: {msg.Level}, Category: {msg.Category}, Message: {msg.Message}");
#endif
                    break;

                case LogLevel.Warning:
#if UNITY_EDITOR
                    Debug.LogWarning($"<color=#00b7ff>[EOSTransport]</color> Level: {msg.Level}, Category: {msg.Category}, Message: {msg.Message}");
#else
                    Debug.LogWarning($"[EOSTransport] Level: {msg.Level}, Category: {msg.Category}, Message: {msg.Message}");
#endif
                    break;

...
namespace EpicTransport
{
    ...
    public class EOSTransport : Transport
    {
        ...
        [Tooltip("Max allowed fragments sent to this player at once before kicking the connection.\n\nIncrease if you are sending more than 5.8KB per packet.")] public int maxFragments = 5;
        ...
        public static bool ConnectedToLobby => manager != null && manager.mode != NetworkManagerMode.Offline && manager.mode != NetworkManagerMode.ServerOnly;
        ...
        #region Client
        public override bool ClientConnected() => client != null && client.Active && !client.ShuttingDown;
        ...
        public override bool ServerActive() => server != null && server.Active && !server.ShuttingDown;
        ...
        public override void ServerStart()
        {
            ...
            if (NetworkManager.singleton.mode == NetworkManagerMode.ServerOnly) { TransportLogger.LogError("Server only mode is not supported with EOSTransport."); Shutdown(); return; }
        ...
        {
            if (!EOSManager.Initialized) return;
            if (string.IsNullOrEmpty(id)) throw new ArgumentNullException(nameof(id), "Lobby ID cannot be null.");

            id = Helper.ToEOSString(id);
    ...
    public class LobbyInfo
    {
        ...
        public async Task<bool> SetMaxMembers(int newMaxMembers)
        {
            TaskCompletionSource<bool> tcs = new TaskCompletionSource<bool>();
            if (!IsLobbyOwner)
            {
                TransportLogger.LogWarning("cannot update max lobby members: player is not lobby owner.");
                tcs.SetResult(false);
            }

            LobbyInterface li = EOSManager.GetLobbyInterface();
            ...
            Result res1 = li.UpdateLobbyModification(ref modopt, out LobbyModification mod);
            if (res1 != Result.Success) throw new EOSSDKException(res1, "Failed to get lobby modification when updating max members!");

            LobbyModificationSetMaxMembersOptions setopt = new LobbyModificationSetMaxMembersOptions() { MaxMembers = (uint)newMaxMembers };
            Result res2 = mod.SetMaxMembers(ref setopt);
            if (res2 != Result.Success) throw new EOSSDKException(res2, "Failed to set max members when updating max members!");

            UpdateLobbyOptions updateopt = new UpdateLobbyOptions() { LobbyModificationHandle = mod };
            li.UpdateLobby(ref updateopt, null, (ref UpdateLobbyCallbackInfo cb) =>
            {
                if (cb.ResultCode != Result.Success) throw new EOSSDKException(cb.ResultCode, "Failed to update lobby when updating max members!");
                EOSTransport.instance.UpdateMaxPlayers(newMaxMembers);
